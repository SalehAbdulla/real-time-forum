...
## Directory Structure (SPA Single Page Architecture)

```
├── cmd/
│   └── main.go              # Entry Point: Runs SQLite init, setup WebSocket hub, and starts server
├── pkg/
│   ├── database/
│   │   └── sqlite.go        # Handles table creation logic and configuration parameters
│   ├── models/
│   │   └── structs.go       # Struct mappings for Users, Posts, Comments, Messages, and Reactions
│   ├── handlers/
│   │   ├── auth.go          # Custom handles for user creation, login sessions, and validation
│   │   ├── forum.go         # API route handlers for Categories, Posts, and Comments
│   │   └── ws.go            # Upgrades protocol to WebSocket and channels clients to the manager hub
│   └── websocket/
│       ├── hub.go           # Manages online statuses, registers connections, and routes traffic
│       └── client.go        # Reads and writes unique payload channels per active browser instance
├── static/
│   ├── css/
│   │   └── style.css        # Visual styles, flex boxes, active chat frames
│   └── js/
│       ├── app.js           # Core SPA Router: Intercepts URL fragments and changes structural state
│       ├── ws-client.js     # Manages front-end socket loops, message queues, and user indicators
│       └── components.js    # Logic handling for inputs, scroll listeners, throttle, and debounce
└── templates/
    └── index.html           # The ONLY HTML file in the layout. Container for target injections.
...
* **Throttle / Debounce Module:** The JavaScript layer inside `static/js/components.js` must encapsulate a throttle system attached to the scroll tracking boundaries of the message container element. This ensures that scrolling to the top requests the next 10 items without firing duplicate HTTP queries.
* **DOM Injection Overhaul:** Page transitions do not perform browser refreshes. When components alter state, JavaScript parses localized parameters, renders the output element, and cleanly replaces the context container components inside `index.html`.